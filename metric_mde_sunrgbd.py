...
def metricOneImage(src_file_path, gt_file_path, pred_file_path, camera_label, scene_label, depth_scale = 1000.0):
    ...
    if not os.path.exists(gt_file_path):
        print(gt_file_path,"not exist (gt)")
        return
    ...
    if not os.path.exists(pred_file_path):
        print(pred_file_path, "not exist (pred)")
        return
    ...
    if not os.path.exists(src_file_path):
        print(src_file_path,"not exist (src)")
        return
    ...
    pre_results_cls[cls_label].append(pred_img.ravel())
    gt_results_cls[cls_label].append(gt_img.ravel())
    # print("\n", eval)
...
for line_f in lines_f[:]:
    ...
    dataset_root = file_list_name.split("/")[1]
    src_file_path = os.path.join("data", "sunrgbd", line_list[0])
    gt_file_path = os.path.join("data", "sunrgbd", line_list[1])
    pred_file_path = os.path.join(pred_path, f"data_{dataset_root}_" + line_list[0].replace("/", "_").replace("jpg", "png"))
    camera_label = line_list[0].split("/")[1]
    # SUNRGBD/xtion/sun3ddata/mit_32_g825/g825_1/0000028-000000904932/image/0000028-000000904932.jpg
    # SUNRGBD/xtion/sun3ddata/mit_32_g825/g825_1/0000028-000000904932/scene.txt
    scene_file = open(os.path.join("data", "sunrgbd", line_list[0][:line_list[0].rfind("/")].replace("image", "scene.txt")), "r")
    scene_label = scene_file.readline()
    # print(camera_label, scene_label)

    # 输入为rect，对每个rect输入计算评估结果
    metricOneImage(src_file_path, gt_file_path, pred_file_path, camera_label, scene_label, depth_scale = 8000.0)
...
for key, value in pre_eval_results_cls.items():
    ...
    # 保存图像
    plt.savefig(os.path.join("histogram", f"sunrgbd_{key}_histogram.png"))
    plt.clf()
