...
class BinsFormerDecodeHeadSceneLabelAwareV2(DepthBaseDecodeHead):
    """BinsFormer head
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(BinsFormerDecodeHeadSceneLabelAwareV2, self).__init__(**kwargs)
    ...
    def forward(self, inputs, scene_info_list = []):
        ...
        for item_bin, pred_logit, pred_class in \
            ...
            if self.binsformer is False:
                ...
                n, n_bins = bins.shape
                # bin centers
                left_bin_widths = (scene_info_list[:,3] - scene_info_list[:, 1]).view(n,1) * bins[:, :n_bins-1] / torch.sum(bins[:, :n_bins-1], dim=1).view(n,1)        # (n, n_bins-1)'
                left_bin_widths = torch.cat((scene_info_list[:, 1:2], left_bin_widths), dim=1)
                ...
                centers = torch.cat((left_centers, last_col), dim=1)
                _, dout = centers.size()
